...
def main():
    ...
    f_df=df[(df['hist_var']=='H2A.Z')&(df['taxid']!=240206)]
    ...
    AlignIO.write(msa, "results/h2az_cellular.fasta", "fasta")
    ...
    AlignIO.write(msa_annot, "results/h2az_cellular_annot.fasta", "fasta")
    ...
    #5. Visualize MSA
    aln2html(msa,'results/h2az_cellular.html',features=get_hist_ss_in_aln_for_html(msa,'H2A',0),title="H2A.Z in cellular organisms",description=True,field1w=10,field2w=35)
    ...
    t.render("results/h2az_cellular.svg", w=6000, dpi=300, tree_style=ts)
    ...
    plot_prof4seq('results/h2az_cellular_cons_ent_unw',map(lambda x:log(20)+x,map(float,cons_prof(msa_tr,f=0,c=0))),cn,features,axis='conservation',title='Conservation, H2A.Z cellular organisms')
    
    # plot_prof4seq('cons_sofp_unw',map(float,cons_prof(msa_tr,f=0,c=2)),cn,features,axis='conservation')
    plot_prof4seq('results/h2az_cellular_cons_sofp_unw_renorm1',map(float,cons_prof(msa_tr,f=0,c=2,m=1)),cn,features,axis='conservation',title='Conservation, H2A.Z cellular organisms')
    plot_prof4seq('results/h2az_cellular_cons_sofp_psic_renorm1',map(float,cons_prof(msa_tr,f=2,c=2,m=1)),cn,features,axis='conservation',title='Conservation, H2A.Z cellular organisms')
